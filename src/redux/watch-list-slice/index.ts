...
export const addToWatchList = createAsyncThunk<Movie, Movie, { rejectValue: { id: number; error: string } }>(
  "watchList/add",
  async (movie: Movie, { dispatch, rejectWithValue }) => {
    try {
      ...
      return rejectWithValue({
        id: movie.id,
        error: error instanceof Error ? error.message : "Error adding to watch list",
      });
    }
  }
);

export const removeFromWatchList = createAsyncThunk<number, Movie, { rejectValue: string }>(
  "watchList/remove",
  async (movie: Movie, { dispatch, rejectWithValue }) => {
    try {
      const watchList = JSON.parse(localStorage.getItem("watch-list") || "[]");
      const newWatchList = watchList.filter((item: Movie) => item.id !== movie.id);
      localStorage.setItem("watch-list", JSON.stringify(newWatchList));
      dispatch(addToast(`${movie.title || movie.name} Removed from Watch List`));
      return movie.id;
    } catch (error) {
      return rejectWithValue(error instanceof Error ? error.message : "Error removing from watch list");
    }
  }
);
